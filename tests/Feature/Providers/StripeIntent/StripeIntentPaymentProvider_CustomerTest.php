...
class StripeIntentPaymentProvider_CustomerTest extends TestCase{
    ...
    public function test_can_associate_user_user_info_when_customer_init_payment():Transaction
    {
        ...
        $mockOrder->shouldReceive('getCustomer')
            ->twice()
            ->andReturn(new CustomerData(['user_type'=>$user_type,'user_id'=>$user_id,'email'=>$email])
            );
     ...
     * attached to payment intent when provided during init.
     *
     */
    public function test_can_use_saved_payment_method_when_customer_init_payment()
    {
        $amount = 1000;
        $user_type='type-is-test-class';
        $user_id='21022022';
